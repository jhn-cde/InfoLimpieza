...
export class ReactiveService {
  ...
  private fillStudentsTeachers(rows: any[]){
    try{
      ...
      console.log('Error! in function fillStudentsTeachers');
      this.reset();
    }
  }
  ...
  private getDistribution(){
    try{
      ...
      console.log('Error! in function getDistribution');
      this.reset();
    }
  }

  private reset(){
    this.teachers=[]; this.students = []; this.distribution = []; this.tree = []; this.enrollments = [];
    this.dataSubject.next({distribution: [], students: [], teachers: []});
